...
class SparkDownApp:
    ...
    def setup_styles(self):
        ...
        self.style.map("Toolbar.TButton", background=[("active", self.colors["hover"])])
    ...
    def create_menu(self):
        """Create the menu bar"""
        self.menubar = Menu(
            self.root, bg=self.colors["bg_secondary"], fg=self.colors["text_primary"]
        )
        self.root.config(menu=self.menubar)

        # File menu
        file_menu = Menu(
            self.menubar, tearoff=0, bg=self.colors["bg_secondary"], fg=self.colors["text_primary"]
        )
        ...
        view_menu.add_command(
            label="Toggle View/Edit", accelerator="Ctrl+Tab", command=self.toggle_mode
        )
        view_menu.add_command(
            label="Preview in Browser", accelerator="Ctrl+P", command=self.preview_in_browser
        )
        ...
        for name, fmt in EXPORT_FORMATS:
            export_menu.add_command(
                label=f"Export to {name}", command=lambda f=fmt: self.export_file(f)
            )
    ...
    def create_toolbar(self):
        ...
        self.toolbar = tk.Frame(
            self.root, bg=self.colors["bg_secondary"], height=40, relief=tk.RAISED
        )
        ...
        self.btn_open = tk.Button(
            self.toolbar, text="📂 Open", command=self.open_selected_file, **btn_config
        )
        self.btn_open.pack(side=tk.LEFT, padx=2, pady=2)

        self.btn_save = tk.Button(
            self.toolbar, text="💾 Save", command=self.save_file, **btn_config
        )
        self.btn_save.pack(side=tk.LEFT, padx=2, pady=2)

        self.btn_delete = tk.Button(
            self.toolbar, text="🗑 Delete", command=self.delete_file, **btn_config
        )
        self.btn_delete.pack(side=tk.LEFT, padx=2, pady=2)

        # Separator
        tk.Frame(self.toolbar, bg=self.colors["border"], width=1).pack(
            side=tk.LEFT, fill=tk.Y, padx=5, pady=5
        )

        self.btn_preview = tk.Button(
            self.toolbar, text="👁 Preview", command=self.toggle_mode, **btn_config
        )
        self.btn_preview.pack(side=tk.LEFT, padx=2, pady=2)

        self.btn_browser = tk.Button(
            self.toolbar, text="🌐 Browser", command=self.preview_in_browser, **btn_config
        )
        ...
        self.btn_toggle = tk.Button(
            self.toolbar, text="🔄 Switch to Edit", command=self.toggle_mode, **btn_config
        )
...
"""
    ...
    def new_file(self):
        ...
        if self.has_unsaved_changes:
            if not messagebox.askyesno(
                "Unsaved Changes", "You have unsaved changes. Create new file anyway?"
            ):
                return
    ...
    def load_file(self, file_path):
        ...
        if self.has_unsaved_changes:
            if not messagebox.askyesno(
                "Unsaved Changes", "You have unsaved changes. Open another file anyway?"
            ):
                return
    ...
    def change_folder(self):
        ...
        folder = filedialog.askdirectory(
            title="Select Folder", initialdir=self.file_manager.folder_path
        )
        if folder:
    ...
    def _setup_preview_tags(self):
        ...
        self.preview.tag_configure(
            "code", font=("Consolas", 12), background=bg_tertiary, foreground=code_color
        )
        self.preview.tag_configure(
            "code_block",
            font=("Consolas", 11),
            background=bg_tertiary,
            foreground=code_color,
            relief=tk.FLAT,
            lmargin1=20,
            lmargin2=20,
        )
        ...
        self.preview.tag_configure(
            "blockquote", background=bg_tertiary, lmargin1=20, lmargin2=20, relief=tk.FLAT
        )
        self.preview.tag_configure(
            "blockquote_text", foreground=text_secondary, font=("Segoe UI", 12, "italic")
        )
    ...
    def export_file(self, format_name):
        ...
        filename = filedialog.asksaveasfilename(
            ...
            filetypes=[(f"{format_name.upper()} files", f"*{format_ext}"), ("All files", "*.*")],
            initialdir=self.file_manager.folder_path,
            ...
            try:
                ...
                self.root.after(
                    ...
                    lambda: messagebox.showinfo(
                        "Export Complete", f"File exported successfully to:\n{filename}"
                    ),
                )
                ...
                self.root.after(
                    0,
                    lambda err=e: messagebox.showerror("Export Error", f"Failed to export: {err}"),
                )
