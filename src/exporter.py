...
class Exporter:
    ...
    def _run_pandoc(self, input_content: str, output_path: str, output_format: str) -> bool:
        ...
        try:
            ...
            raise ImportError(
                "pypandoc is required for this export. Install with: pip install pypandoc"
            ) from None
        ...
        for line in lines:
            ...
            if line.startswith("```"):
                if current_block and current_block.get("type") == "code":
                    ...
                    if current_block:
                        blocks.append(current_block)
                    current_block = {"type": "code", "language": line[3:].strip(), "content": ""}
                continue
            ...
            if line.startswith(">"):
                if current_block and current_block.get("type") == "blockquote":
                    ...
                    if current_block:
                        blocks.append(current_block)
                    current_block = {"type": "blockquote", "content": line[1:].strip() + "\n"}
                continue
